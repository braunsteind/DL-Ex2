...
class Part1Model(object):
    ...
    def feedforward_the_dev_set(self, epoch_num, avg_validation_loss_per_epoch_dict,
        ...
        for data, target in self.dev_dataset:
            ...
            y_hat = get_y_tag(output)
            if tagger_type == 'ner':
                if ut1.Representation_Of_Indexes_By_classes[y_hat.cpu().sum().item()] != 'O' or ut1.Representation_Of_Indexes_By_classes[target.cpu().sum().item()] != 'O':
    ...
    def test(self, ner_or_pos):
        ...
        self.create_predictions_file(path_to_test_file, "test1." + ner_or_pos, pred_list)
