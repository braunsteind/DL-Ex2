...
def calculate_loss_print(size_of_batch, model, set, is_training, mapping_type):
    ...
    for data, tag in set:
        ...
        if mapping_type == 'ner' and is_training != 1:
            if my_help_functions.Representation_Of_Indexes_By_classes[y_tag.cpu().sum().item()] != 'O' or \
                    my_help_functions.Representation_Of_Indexes_By_classes[tag.cpu().sum().item()] != 'O':
...
def train_neural_network(model, train, validation_set, mapping_type):
    ...
    second_lable, = plt.plot(dev_accuracy_per_iter.keys(), dev_accuracy_per_iter.values(), "r-",
